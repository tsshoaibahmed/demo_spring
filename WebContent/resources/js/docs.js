...
function updateBorders(color) {	
    ...
    var color1 = '292929';
    var color11 = 'ffffff';
	var color2 = color.toString().replace('#','');
	var ratio = 0.5;
	var hex = function(x) {
	    x = x.toString(16);
	...
	var r1 = Math.ceil(parseInt(color11.substring(0,2), 16) * ratio + parseInt(color2.substring(0,2), 16) * (1-ratio));
	var g1 = Math.ceil(parseInt(color11.substring(2,4), 16) * ratio + parseInt(color2.substring(2,4), 16) * (1-ratio));
	var b1 = Math.ceil(parseInt(color11.substring(4,6), 16) * ratio + parseInt(color2.substring(4,6), 16) * (1-ratio));

    
	var middle = hex(r) + hex(g) + hex(b);
	var hover = hex(r1) + hex(g1) + hex(b1);	
	//alert(middle);
   
    $('body').removeClass('bg-dark');
    $('nav').removeClass('bg-dark');
   
   $('#mainNav.navbar-dark .navbar-collapse .navbar-sidenav').css('background','none');
  
   $('body').css('background-color',hexColor);
   $('nav').css('background-color',hexColor);
   $('#accordion').css('background-color',hexColor);
   $('.sidenav-second-level').css('background-color',hexColor);
   $('.sidenav-third-level').css('background-color',hexColor);
   $('.sidenav-fourth-level').css('background-color',hexColor);
   /* jss.set('body', {
...
*/
    $('.card-header').css({'background': 'linear-gradient(91deg, '+hexColor+' 0%, '+hexColor+''+"4D"+' 100%)','border-bottom': '1px  solid' +hexColor});
    $('.card-body').css({background:hexColor+"4D"});
    $('.card-body .card .card-body').css({background:'transparent'});
    $('.form-control').css({border:'1px solid' +hexColor+"99"});
    $('hr').css({border:'1px solid' +hexColor+"66"});
    $('.input-group-text').css({"border":'1px solid' +hexColor+'99',"background-color":'transparent'});
    $('.card').css({"box-shadow": '0px 0px 0px 5px' +hexColor+', 0px 4px 20px rgb(2, 1, 1)'});
    $('.card-body .card').css({"box-shadow": 'none','border':'1px solid' +hexColor});
  //  $('#mainNav.navbar-dark .navbar-collapse .navbar-sidenav li.active a').css({'background':''});
    $('#mainNav.navbar-dark .navbar-collapse .navbar-sidenav > .nav-item .sidenav-third-level > li > a:hover').css({'background':'#'+middle});
     $('#mainNav.navbar-dark .navbar-collapse .navbar-sidenav li.active a').css({'background':'linear-gradient(91deg, '+'#'+hover+''+'4D'+' 0%, '+'#'+hover+' 100%)'});
     $('#menubar').css('background-color','transparent');
     $('.modal-header').css({'background': 'linear-gradient(91deg, '+hexColor+' 0%, '+hexColor+''+"4D"+' 100%)','border-bottom': '1px  solid' +hexColor});
     $('.modal-body').css({background:hexColor+"4D"});
     $('.form-control:focus').css({'box-shadow': '0 0 0 .2rem' +'#'+middle});
     $('.close').css('color','#'+middle);
   // $('nav ul').removeClass('bg-dark');
    // $('#mainNav.fixed-top.navbar-dark .sidenav-toggler').css('background-color', hexColor);
     $('footer.sticky-footer').css('background-color', hexColor);
    //$('nav').removeClass('bg-dark');
...
/*    showInput: true,
    ...
    palette: [
        [ "rgb(67, 67, 67)", "rgb(102, 102, 102)", "rgb(153, 153, 153)","rgb(183, 183, 183)",
    ...
    palette: [
        ...
        ["#f90","#444","#666","#999","#674ea7"],        
        ["#e69138","#6aa84f","#45818e","#3d85c6","#a64d79"],
        ["#b45f06","#bf9000","#38761d","#0b5394","#741b47"]
